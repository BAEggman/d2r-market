...
// Gemini 스크린샷 인식 — 무료 티어에서 쓸 수 있는 모델을 앞에서부터 시도합니다.
const AI_MODELS = [
...
const MAX_IMAGE_CHARS = 6 * 1024 * 1024; // base64 기준 6MB
...
async function route(request, env) {
  ...
  // ── 스크린샷 인식(AI) ──
  if (p === "/ai/identify" && m === "POST") return identifyItem(request, env);
...
const AI_PROMPT = [
  ...
  "The game client may be in Korean or English; most likely Korean.",
  "",
  "STEP 0 - TRANSCRIBE THE NAME. This is the most important field.",
  "Copy the item name line into nameRaw and the line under it into baseRaw,",
  ...
  "",
  "The game receives expansions with items you have never seen. If a name is",
  "unfamiliar, that almost certainly means it is a NEW item, not a misspelling",
  "of one you know. NEVER substitute a similar-sounding item you do know.",
  "  기드의 내기 is NOT 기드의 행운 (Gheeds Fortune).",
  "  감시자의 빛 is NOT 무지개 자락 (Rainbow Facet).",
  "",
  "STEP 1 - Determine the item rarity from the colour of its NAME text:",
  ...
  "A RARE item name is randomly generated by combining two affix words.",
  "Korean examples: 해골의 눈, 죽음의 손아귀, 공포의 문장.",
  "These are NOT unique items and do NOT exist in any item list. NEVER map a",
  "rare item name to a unique item, no matter how similar it looks.",
  "",
  "STEP 3 - Provide up to 3 candidate identifications in names, best first:",
  "- unique/set/runeword/rune: the item official name, BOTH in Korean (ko)",
  "    and English (en) if you know them.",
  "- rare/magic/crafted/normal: the BASE TYPE name, both Korean and English",
  "    (from the line under the item name).",
  "If unsure, still provide your best guess and set a low confidence (<=0.3).",
  "",
  "KOREAN CLASS NAMES - translate/recognise these EXACTLY, never guess from sound:",
  "  아마존->Amazon 암살자->Assassin 강령술사->Necromancer 야만용사->Barbarian",
  "  성기사->Paladin 원소술사->Sorceress 드루이드->Druid",
  "  악마술사->Warlock (NOT Necromancer, a separate class from the",
  "  Reign of the Warlock expansion). Warlock trees: 이단(Eldritch) 악마(Demon) 혼돈(Chaos).",
  "",
  "STEP 4 - Transcribe the item option/affix lines into optionLines, one",
  "string per line, VERBATIM in the original language (Korean), exactly as",
  "printed including numbers and percent signs, e.g.:",
  "  +2 악마술사 기술",
  "  달리기/걷기 속도 +30%",
  "  적중당 생명력 7% 훔침",
  "  모든 저항 +10",
  "Skip the requirement lines (요구 레벨/요구 힘/요구 민첩), the base",
  "defense/damage line, and the item own name/base-type lines. Keep at most",
  "14 lines.",
  "",
  "Also transcribe every readable line of text in the image into the text",
  "field, one line per line, keeping the original language (fallback only).",
].join("\n");
  ...
  properties: {
    ...
    names: {
      ...
      items: {
        type: "OBJECT",
        properties: {
          ko: { type: "STRING", description: "Korean official name" },
          en: { type: "STRING", description: "English official name" },
          confidence: { type: "NUMBER", description: "0 to 1" },
        },
        required: ["confidence"],
      },
    },
    optionLines: { type: "ARRAY", items: { type: "STRING" } },
    text: { type: "STRING", description: "All readable text in the image" },
  },
  required: ["nameRaw", "baseRaw", "rarity", "names", "optionLines", "text"],
};
...
async function identifyItem(request, env) {
  ...
  await limiter(env, "ai:" + clientIp(request), 20, 3600);

  const b = await readJson(request);
  const data = b && b.data;
  const mime = (b && b.mime) || "image/jpeg";
  if (typeof data !== "string" || data.length < 32) fail(400, "이미지 데이터가 없습니다");
  if (data.length > MAX_IMAGE_CHARS) fail(413, "이미지가 너무 큽니다. 더 작게 잘라서 올려주세요");
  if (!/^image\/(png|jpeg|webp|gif)$/.test(mime)) fail(400, "지원하지 않는 이미지 형식입니다");

  const payload = {
    contents: [{ role: "user", parts: [{ text: AI_PROMPT }, { inline_data: { mime_type: mime, data } }] }],
    generationConfig: { temperature: 0, responseMimeType: "application/json", responseSchema: AI_SCHEMA },
  };

  const models = [];
  if (env.GEMINI_MODEL) models.push(env.GEMINI_MODEL);
  for (const mdl of AI_MODELS) if (!models.includes(mdl)) models.push(mdl);

  let lastStatus = 0, lastDetail = "";
  const combos = [];
  for (const model of models) for (const ver of ["v1beta", "v1"]) combos.push([model, ver]);
  ...
  for (const [model, ver] of combos) {
    ...
    if (r.ok) {
      ...
      const names = Array.isArray(parsed.names)
        ? parsed.names
            .filter(n => n && (typeof n.ko === "string" || typeof n.en === "string"))
            .slice(0, 3)
            .map(n => ({ ko: str(n.ko).trim(), en: str(n.en).trim(), confidence: Number(n.confidence) || 0 }))
        : [];
      const optionLines = Array.isArray(parsed.optionLines)
        ? parsed.optionLines.filter(s => typeof s === "string" && s.trim()).slice(0, 14).map(s => s.trim())
        : [];
      return json({
        ok: true,
        model,
        nameRaw: str(parsed.nameRaw).trim(),
        baseRaw: str(parsed.baseRaw).trim(),
  ...
  if (lastStatus === 401 || lastStatus === 403) {
    return json({ error: "bad_api_key", message: "Gemini API 키가 거부됐습니다. 키가 올바른지 확인해 주세요." }, 502);
  }
  return json({
